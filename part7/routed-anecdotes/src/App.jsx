...
const Anec = ({ anec }) => {
  ...
  return (
    <div>
      <p><strong>{anec.content}</strong></p>
      <p>has {anec.votes} votes</p>
      <p>for more info see <a href={anec.info} target='_blank'>{anec.info}</a></p>
    </div>
  )
}
...
const CreateNew = (props) => {
  const { reset: contentRes, ...contentProps } = useField('text')
  const { reset: authorRes, ...authorProps } = useField('text')
  const { reset: infoRes, ...infoProps } = useField('text')
  ...
  return (
    <div>
      ...
      <Routes> 
        ...
        <Route path='/anecdotes/:id' element={<Anec anec={anec} />} />
      </Routes>
      <Footer />
    </div>
  )
